...
export default function SignUp({ onBack }: { onBack: () => void }) {
  ...
  const submit = async () => {
    ...
    // Otherwise the app switches to the dashboard automatically (auth state changes).
  };
    ...
    <View style={{ flex: 1, flexDirection: wide ? 'row' : 'column', backgroundColor: theme.color.bg }}>
      ...
      >
        ...
        <Text style={{ color: theme.color.mutedOnDark, fontSize: theme.font.body, maxWidth: 420 }}>
          Join {CONFIG.teamName}. Sign in from any device — your phone or the office — and everyone on the team stays in sync.
        </Text>
      ...
      <View style={{ flex: wide ? 1 : undefined, padding: wide ? 56 : 24, justifyContent: 'center', alignItems: 'center' }}>
        <Card style={{ width: '100%', maxWidth: 420 }}>
          ...
          <Text style={{ color: theme.color.muted, marginTop: 4, marginBottom: 16 }}>Use your work email so your team can find you.</Text>
          <View style={{ gap: 12 }}>
            <Field label="Full name" value={name} onChangeText={setName} placeholder="e.g. Alex Rivera" />
            <Field label="Work email" value={email} onChangeText={setEmail} placeholder="you@onehorizonhomes.com" keyboardType="email-address" />
            <Field label="Company" value={company} onChangeText={setCompany} placeholder="One Horizon Homes" />
            <View style={{ flexDirection: 'row', gap: 12 }}>
              <View style={{ flex: 1 }}><Field label="Password" value={password} onChangeText={setPassword} placeholder="••••••••" secureTextEntry /></View>
              <View style={{ flex: 1 }}><Field label="Confirm" value={confirm} onChangeText={setConfirm} placeholder="••••••••" secureTextEntry /></View>
            </View>
            {error ? <Text style={{ color: theme.color.text, fontWeight: '700', fontSize: theme.font.small }}>⚠ {error}</Text> : null}
            {notice ? <Text style={{ color: theme.color.text, fontWeight: '700', fontSize: theme.font.small }}>✓ {notice}</Text> : null}
            <Button title={busy ? 'Creating…' : 'Create account'} variant="primary" icon="→" onPress={submit} />
